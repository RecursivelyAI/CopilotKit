...
<PropertyReference name="action" type="Action" required>
  The function made available to the Copilot. See [Action](#action).
    ...
    <PropertyReference name="retryOnFailure" type="boolean" default="false">
        Whether to retry the action when failed. If set to `true`. The LLM will retry as it sees fit.
    </PropertyReference>
